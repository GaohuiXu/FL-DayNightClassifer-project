...
> O-127 approves exactly one replacement that reserves one GH200 while forcing
> the process to expose zero CUDA devices; the gate remains CPU-only, with
> 4 CPUs, 32 GiB and 15 minutes. STOP-A A3 is active under that amendment;
...
| STOP-A/B/C implementation, commit, isolated review topology, Slurm tuple and cumulative budget | O-127 authorizes one explicit-one-GPU/CUDA-hidden STOP-A replacement and A4 review; STOP-B/C remain unstarted |

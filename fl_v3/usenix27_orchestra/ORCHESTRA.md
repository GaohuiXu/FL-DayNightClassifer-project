...
> L-A0 completed their exact B4 `D_low` epochs and `D_select` evaluations, but
...
> `FAIL/INCOMPLETE`, its no-retry allocation is consumed, and review is pending.
...
| STOP-A/B/C implementation, commit, isolated review topology, Slurm tuple and cumulative budget | STOP-A closed; STOP-B closed `INCONCLUSIVE`; O-131 C0 Job `492525` is `FAIL/INCOMPLETE`, its sole no-retry allocation is consumed, and review is pending; later C remains owner-gated |

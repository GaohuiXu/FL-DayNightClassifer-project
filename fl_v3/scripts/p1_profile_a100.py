...
This is a SMOKE/diagnostic (Rule #3 — not a scientific result). Determinism-neutral (StepProfiler
only sync()s + reads the clock). Run as a SHORT SLURM job (<<3h): sbatch run_p1_profile_a100.sh.
"""
...
def _build(run_config, device):
    ...
    cdata = task.client_data(0, run_config)        # any real client's loader → representative batch shapes
    ...
    opt = torch.optim.Adam(groups, lr=base_lr)
    return model, crit, cdata.trainloader, opt, len(bb), len(rest)
...
def profile_regime(run_config, device, steps, warmup, label, instrument=True):
    """One regime: per-module fwd + loop phases + util + peak mem (+ wall step time)."""
    model, crit, loader, opt, n_bb, n_rest = _build(run_config, device)
    ...
    res = {
        "label": label,
        "batch_size": int(run_config.get("batch-size")),
        "activation_ckpt": truthy(run_config.get("det-activation-checkpoint", False)),
        "use_amp": use_amp,
        "n_backbone_tensors": n_bb, "n_rest_tensors": n_rest,
        "mean_step_ms": (1e3 * sum(wall) / len(wall)) if wall else None,
        "util": sampler.report(),
        "peak_mem_mib": round(max_mem_mb(), 1),
        "oom": oom,
    }
    if prof is not None:
...
def kernel_breakdown(run_config, device, steps=6, warmup=3):
    """torch.profiler CUDA-kernel breakdown → attributes BACKWARD by kernel + compute-vs-launch."""
    from torch.profiler import ProfilerActivity, profile
    model, crit, loader, opt, _, _ = _build(run_config, device)
    use_amp = (not torch.backends.cudnn.deterministic) and device.type == "cuda"
    grad_clip = float(run_config.get("grad-clip-norm", 0.0))
    model.train()
    it = iter(loader)
    ...
    total_cuda = sum(_cuda_us(e) for e in ka) / 1e3   # us → ms
    ...
    if device.type == "cuda":
        torch.cuda.empty_cache()
    return {"steps": steps, "total_self_cuda_ms": round(total_cuda, 1),
...
def main():
    ...
    print(f"[p1-profile] device={torch.cuda.get_device_name(0)} precision={precision_state()['precision']} "
          f"config={a.config}", flush=True)

    results = {"device": torch.cuda.get_device_name(0), "precision": precision_state()["precision"],
               "steps": a.steps, "warmup": a.warmup, "regimes": [], "kernel_breakdown": None}

    # R1: main teardown — batch 16 (config default), ckpt ON, full instrumentation.
    cfg1 = dict(base); cfg1["det-activation-checkpoint"] = True
    print("[p1-profile] R1: per-component teardown (batch=%s, ckpt=ON)…" % cfg1.get("batch-size"), flush=True)
    results["regimes"].append(profile_regime(cfg1, device, a.steps, a.warmup, "ckpt_on", instrument=True))

    # R1b: kernel breakdown (attributes backward) — same regime.
    print("[p1-profile] R1b: torch.profiler kernel breakdown…", flush=True)
    try:
        results["kernel_breakdown"] = kernel_breakdown(cfg1, device, steps=6, warmup=3)
    except Exception as e:
        results["kernel_breakdown"] = {"error": str(e).splitlines()[0][:160]}

    # R2: activation-ckpt OFF (overhead + does it fit?).
    cfg2 = dict(base); cfg2["det-activation-checkpoint"] = False
    print("[p1-profile] R2: ckpt OFF (overhead / OOM check)…", flush=True)
    results["regimes"].append(profile_regime(cfg2, device, max(8, a.steps // 2), a.warmup, "ckpt_off", instrument=False))
    ...
    for bs in [int(x) for x in a.batch_sizes.split(",")]:
        ...
        results["regimes"].append(profile_regime(cfgb, device, max(8, a.steps // 2), a.warmup, f"batch_{bs}", instrument=False))

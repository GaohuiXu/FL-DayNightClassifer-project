#!/bin/bash
# One-shot O-130 STOP-B fixed/varying-seed decomposition. No optimizer/update.
set -euo pipefail
umask 077

: "${S10_STOPB_RAND_SNAPSHOT:?required}"
: "${S10_STOPB_RAND_OUTPUT:?required}"
: "${S10_STOPB_RAND_EXPECTED_SOURCE_SHA:?required}"
: "${S10_STOPB_RAND_EXPECTED_TREE:?required}"
: "${S10_STOPB_RAND_EXPECTED_RUNNER_SHA256:?required}"
: "${S10_STOPB_RAND_EXPECTED_OBSERVER_SHA256:?required}"
: "${S10_STOPB_RAND_EXPECTED_CAMERA_CONFIG_SHA256:?required}"
: "${S10_STOPB_RAND_EXPECTED_LIDAR_CONFIG_SHA256:?required}"
: "${S10_STOPB_RAND_EXPECTED_FUSION_CONFIG_SHA256:?required}"

RUNNER_REL="fl_v3/scripts/run_s10_stop_b_randomness.sh"
OBSERVER_REL="fl_v3/scripts/s10_stop_b_randomness.py"
CAMERA_REL="fl_v3/configs/s10_b_rand_c_fp32.json"
LIDAR_REL="fl_v3/configs/s10_b_rand_l_fp32.json"
FUSION_REL="fl_v3/configs/s10_b_fp32.json"
...
WORK="${S10_STOPB_RAND_OUTPUT}.control"

test -d "${S10_STOPB_RAND_SNAPSHOT}"
test ! -e "${S10_STOPB_RAND_OUTPUT}"
test ! -e "${WORK}"
for relative in \
  "${RUNNER_REL}" \
  "${OBSERVER_REL}" \
  ...
  "${LIDAR_REL}" \
  "${FUSION_REL}"; do
  test -f "${S10_STOPB_RAND_SNAPSHOT}/${relative}"
done
test -f "${SPLIT_MANIFEST}"
test -f "${PANEL_MANIFEST}"
test -f "${ZIP_MANIFEST}"

actual_source_sha="$(git -C "${S10_STOPB_RAND_SNAPSHOT}" rev-parse HEAD)"
actual_tree="$(git -C "${S10_STOPB_RAND_SNAPSHOT}" rev-parse 'HEAD^{tree}')"
actual_runner_sha256="$(sha256sum "${S10_STOPB_RAND_SNAPSHOT}/${RUNNER_REL}" | cut -d' ' -f1)"
actual_observer_sha256="$(sha256sum "${S10_STOPB_RAND_SNAPSHOT}/${OBSERVER_REL}" | cut -d' ' -f1)"
actual_camera_sha256="$(sha256sum "${S10_STOPB_RAND_SNAPSHOT}/${CAMERA_REL}" | cut -d' ' -f1)"
actual_lidar_sha256="$(sha256sum "${S10_STOPB_RAND_SNAPSHOT}/${LIDAR_REL}" | cut -d' ' -f1)"
actual_fusion_sha256="$(sha256sum "${S10_STOPB_RAND_SNAPSHOT}/${FUSION_REL}" | cut -d' ' -f1)"
test "${actual_source_sha}" = "${S10_STOPB_RAND_EXPECTED_SOURCE_SHA}"
test "${actual_tree}" = "${S10_STOPB_RAND_EXPECTED_TREE}"
test "${actual_runner_sha256}" = "${S10_STOPB_RAND_EXPECTED_RUNNER_SHA256}"
test "${actual_observer_sha256}" = "${S10_STOPB_RAND_EXPECTED_OBSERVER_SHA256}"
test "${actual_camera_sha256}" = "${S10_STOPB_RAND_EXPECTED_CAMERA_CONFIG_SHA256}"
test "${actual_lidar_sha256}" = "${S10_STOPB_RAND_EXPECTED_LIDAR_CONFIG_SHA256}"
test "${actual_fusion_sha256}" = "${S10_STOPB_RAND_EXPECTED_FUSION_CONFIG_SHA256}"
test "$(sha256sum "${SPLIT_MANIFEST}" | cut -d' ' -f1)" = "${SPLIT_SHA256}"
test "$(sha256sum "${PANEL_MANIFEST}" | cut -d' ' -f1)" = "${PANEL_FILE_SHA256}"
test -z "$(git -C "${S10_STOPB_RAND_SNAPSHOT}" status --short --untracked-files=all)"
test "$(git -C "${S10_STOPB_RAND_SNAPSHOT}" branch --show-current)" = ""

# shellcheck disable=SC1091
source "${S10_STOPB_RAND_SNAPSHOT}/fl_v3/scripts/arrhenius_env.sh"
arrhenius_load_modules build
module load nuScenes-data/1.0-map-1.3-zip
arrhenius_activate_env

export PYTHONPATH="${S10_STOPB_RAND_SNAPSHOT}/fl_v3/src"
...
finalize() {
  local status="${1:-1}"
  local target="${S10_STOPB_RAND_OUTPUT}"
  if [[ ! -d "${target}" ]]; then
    mv "${WORK}" "${target}"
  elif [[ -d "${WORK}" ]]; then
...
if platform.machine() != "aarch64":
    raise RuntimeError("STOP-B B-RAND requires an aarch64 compute node")
if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
    raise RuntimeError("STOP-B B-RAND requires exactly one visible CUDA device")
name = torch.cuda.get_device_properties(0).name
if "GH200" not in name.upper():
    raise RuntimeError(f"STOP-B B-RAND expected GH200, got {name!r}")
print(json.dumps({
    "schema": "fl_v3.s10.stop_b_rand_resource_identity.v1",
...
set +e
python "${OBSERVER_REL}" \
  --camera-config "${CAMERA_REL}" \
  --lidar-config "${LIDAR_REL}" \
  --fusion-config "${FUSION_REL}" \
  ...
  --output-dir "${S10_STOPB_RAND_OUTPUT}" \

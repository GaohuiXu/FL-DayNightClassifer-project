...
require(spec["schema_version"] == "s10.phase1.envelope_b_dual.v1", "schema")
require(spec["request_state"] == "frozen_owner_activation_required", "state")
require(spec["branch"] == "codex/s10-phase1p-throughput-preflight", "branch")
require(spec["candidate_count"] == 2, "candidate count")
require(spec["serial_order"] == ["lidar", "camera"], "serial order")
require(spec["seed_policy"] == [0], "seed policy")
require(spec["aggregate_resource"]["max_concurrency"] == 1, "concurrency")

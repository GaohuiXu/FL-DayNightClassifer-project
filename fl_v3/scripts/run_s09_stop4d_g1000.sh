#!/bin/bash
# One-shot optimized S09 STOP-4D F-U G1000. No loader matrix or profiler.
set -euo pipefail
umask 077

: "${S09_STOP4D_SNAPSHOT:?required}"
: "${S09_STOP4D_OUTPUT:?required}"
: "${S09_STOP4D_EXPECTED_SOURCE_SHA:?required}"
: "${S09_STOP4D_EXPECTED_TREE:?required}"
: "${S09_STOP4D_EXPECTED_CONFIG_SHA256:?required}"

CONFIG_REL="fl_v3/configs/s09_stop4d_f_u_g1000.json"
RUNNER_REL="fl_v3/scripts/run_s09_stop4d_g1000.sh"
TRAIN_REL="fl_v3/scripts/centralized_train.py"
READINESS_DIR="${S09_STOP4D_OUTPUT}/readiness"

test -d "${S09_STOP4D_SNAPSHOT}"
test ! -e "${S09_STOP4D_OUTPUT}"
for relative in "${CONFIG_REL}" "${RUNNER_REL}" "${TRAIN_REL}"; do
  test -f "${S09_STOP4D_SNAPSHOT}/${relative}"
done

actual_source_sha="$(git -C "${S09_STOP4D_SNAPSHOT}" rev-parse HEAD)"
actual_tree="$(git -C "${S09_STOP4D_SNAPSHOT}" rev-parse 'HEAD^{tree}')"
test "${actual_source_sha}" = "${S09_STOP4D_EXPECTED_SOURCE_SHA}"
test "${actual_tree}" = "${S09_STOP4D_EXPECTED_TREE}"
test -z "$(git -C "${S09_STOP4D_SNAPSHOT}" status --short --untracked-files=all)"
test "$(git -C "${S09_STOP4D_SNAPSHOT}" branch --show-current)" = ""

# shellcheck disable=SC1091
source "${S09_STOP4D_SNAPSHOT}/fl_v3/scripts/arrhenius_env.sh"
arrhenius_load_modules build
module load nuScenes-data/1.0-map-1.3-zip
arrhenius_activate_env

export PYTHONPATH="${S09_STOP4D_SNAPSHOT}/fl_v3/src"
...
mkdir -p "${S09_STOP4D_OUTPUT}"
cd "${S09_STOP4D_OUTPUT}"

python - "${S09_STOP4D_SNAPSHOT}/${CONFIG_REL}" <<'PY' > config_identity.json
import hashlib
import json
import os
import sys
...
expected = os.environ["S09_STOP4D_EXPECTED_CONFIG_SHA256"]
...
if data["model"]["camera_activation_checkpoint"] is not False:
    raise RuntimeError("STOP-4D requires camera activation checkpointing disabled")
if data["training"]["max_optimizer_steps"] != 1000:
    raise RuntimeError("STOP-4D requires exactly 1000 successful optimizer updates")
if data["execution"]["max_attempted_windows"] != 1020:
    raise RuntimeError("STOP-4D requires the frozen 1020 attempted-window cap")
if data["execution"]["timing_warmup_successful_windows"] != 10:
    raise RuntimeError("STOP-4D requires ten accepted warm-up windows")
if data["execution"]["loader_profile"] is not None:
    raise RuntimeError("STOP-4D forbids the retired worker matrix")
if data["execution"]["operator_profile"] is not None:
    raise RuntimeError("STOP-4D forbids an operator profiler")
print(json.dumps({
...
python - "${S09_STOP4D_SNAPSHOT}" "${actual_source_sha}" "${actual_tree}" \
  "${S09_STOP4D_SNAPSHOT}/${RUNNER_REL}" <<'PY' > execution_identity.json
import hashlib
import json
import os
import platform
...
if platform.machine() != "aarch64":
    raise RuntimeError("S09 STOP-4D requires an aarch64 compute node")
if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
    raise RuntimeError("S09 STOP-4D requires exactly one visible CUDA device")
if torch.cuda.get_device_name(0) != "NVIDIA GH200 120GB":
    raise RuntimeError("S09 STOP-4D requires the reviewed NVIDIA GH200 120GB target")
print(json.dumps({
    "schema": "s09.stop4d-execution-identity.v1",
    ...
    "resolved_config_sha256": os.environ["S09_STOP4D_EXPECTED_CONFIG_SHA256"],
...
set +e
python -m pytest -q \
  "${S09_STOP4D_SNAPSHOT}/fl_v3/tests/test_s06_resolved_config.py" \
  "${S09_STOP4D_SNAPSHOT}/fl_v3/tests/test_s09_readiness.py" \
  "${S09_STOP4D_SNAPSHOT}/fl_v3/tests/test_s08_precision_diagnostics.py" \
  "${S09_STOP4D_SNAPSHOT}/fl_v3/tests/test_s08_precision_partition.py" \
  "${S09_STOP4D_SNAPSHOT}/fl_v3/tests/test_s07_b_integration.py::test_resolved_camera_checkpoint_switch_is_explicit_and_boolean" \
  "${S09_STOP4D_SNAPSHOT}/fl_v3/tests/test_s07_b_integration.py::test_multitask_loss_term_recording_is_output_and_gradient_neutral" \
...
start_ns="$(date '+%s%N')"
set +e
python "${S09_STOP4D_SNAPSHOT}/${TRAIN_REL}" \
  --config "${S09_STOP4D_SNAPSHOT}/${CONFIG_REL}" \
...
if (( centralized_status == 0 )); then
  set +e
  python - "${S09_STOP4D_SNAPSHOT}" "${READINESS_DIR}/readiness.json" <<'PY' \
...
if report.get("target_successful_windows") != 1000:
    errors.append("STOP-4D target-successful-window count drifted")
if report.get("max_attempted_windows") != 1020:
    errors.append("STOP-4D attempted-window cap drifted")
...
result = {
    "schema": "s09.stop4d-readiness-validation.v1",
    "status": "PASS" if report.get("status") == "PASS" and not errors else "FAIL",
    "evidence_validation_errors": errors,
    "performance_gates": {
        **performance["metrics"],
        "accepted_stop4c_combined_p50_ms": 183.2151461392641,
        "accepted_stop4c_combined_p95_ms": 217.6735214941669,
        "material_regression_stop3_p50_limit_ms": 229.620313,
        "material_regression_stop3_p95_limit_ms": 246.759346,
    },
}
...
result = {
    "schema": "s09.stop4d-telemetry-alignment.v1",
...
artifact_manifest_tmp="${S09_STOP4D_OUTPUT}.artifact_sha256s.$$"

#!/bin/bash
# Exact single-GH200 Camera-B16 trace and conservative same-allocation screen.
set -euo pipefail
...
fail() { echo "[s10-phase1p-ip-e3] ERROR: $*" >&2; exit 2; }
...
trace_output="${expected_output_root}/camera/trace_${source_sha:0:12}_r1_b16_trace"
reference_output="${expected_output_root}/camera/sustained_${source_sha:0:12}_r1_b16_reference"
candidate_output="${expected_output_root}/camera/sustained_${source_sha:0:12}_r1_b16_batched_affine"
pair_output="${expected_output_root}/pairs/b16_reference_vs_batched_affine.json"
...
python -m pytest -q \
  ...
  fl_v3/tests/test_s10_phase1p_profile.py::test_ip_e3_production_runtime_is_not_patched_twice \
  fl_v3/tests/test_s10_phase1p_profile.py::test_camera_trace_diagnosis_requires_preprocess_to_rank_first \
  fl_v3/tests/test_s03_camera_contract.py::test_s10_phase1p_batched_affine_grid_is_elementwise_exact_on_cpu_and_cuda \
  fl_v3/tests/test_s10_phase1p_compare.py::test_b16_followup_near_neutral_gate_unlocks_implementation_only
...
run_one trace "${reference_profile}" "${trace_output}" b16_trace
trace_continue="$(python - "${trace_output}/torch_trace_summary.json" <<'PY'
import json, sys
with open(sys.argv[1], encoding="utf-8") as stream:
    summary = json.load(stream)
diagnosis = summary["camera_stage_diagnosis"]
print("1" if diagnosis["preprocess_is_largest_camera_forward_range"] else "0")
PY
)"
if [[ "${trace_continue}" != "1" ]]; then
  echo "[s10-phase1p-ip-e3] TRACE_STOP: preprocessing is not the largest named Camera forward range"
  exit 0
fi

run_one sustained "${reference_profile}" "${reference_output}" b16_reference
run_one sustained "${candidate_profile}" "${candidate_output}" b16_batched_affine

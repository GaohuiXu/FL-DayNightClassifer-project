...
# D16 regime (precision=bf16 by default; must match the checkpoint, batch_size=1 canonical decode). Serves BOTH:
...
PRECISION="${PRECISION:-bf16}"      # D16 single knob: bf16 (science) | fp32 (dev/deterministic). Must match the checkpoint's regime.
...
echo "===== readiness eval (precision=${PRECISION} diagnostic=${DIAGNOSTIC:-0}) =====  node=$(hostname) job=${SLURM_JOB_ID:-local}"
...
python3 -c "import json;d=json.load(open('${OUT}/benchmark_readiness.json'));print(d['verdict'],'scope',d.get('verdict_scope'),'precision',d.get('precision'),'| mAP',round(d['mAP'],4),'NDS',round(d['NDS'],4),'car_recall',round(d['official_clean_car_recall'],4),'eligible_N',d['eligible_count'],'false_disappear',d['false_disappearance'].get('false_disappearance_rate'),'| hash',d['frozen_subset_hash'][:16])" || true

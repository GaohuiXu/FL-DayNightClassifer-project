...
SNAP_FLAG=""; [ -n "${SNAPSHOT_EPOCHS:-}" ] && SNAP_FLAG="--snapshot-epochs ${SNAPSHOT_EPOCHS}"
...
# 1 BLAS/OMP thread per process: forked DataLoader workers inherit this pre-import, so with 4 ranks × 8
# workers on a 32-CPU node we get 32 (not 32×8=256) BLAS threads — kills the ego-compensation matmul
# oversubscription that dominated the multi-sweep loader. Value-neutral (timing only). seeded_worker_init
# also pins torch.set_num_threads(1) per worker; GPU is the compute path so the rank's CPU stays fast enough.
export OMP_NUM_THREADS=1
export OPENBLAS_NUM_THREADS=1
export MKL_NUM_THREADS=1
if ! type module >/dev/null 2>&1; then [ -f /usr/share/lmod/lmod/init/bash ] && source /usr/share/lmod/lmod/init/bash; fi
module purge
# --ignore_cache fallback: some A100 nodes carry a stale Lmod cache that can't see the module (it IS the
# default on the login node) — the cache-bypass resolves it. Both must fail to abort (set -e via the ||-chain).
...
torchrun --standalone --nproc_per_node="${NPROC}" fl_v3/scripts/centralized_train.py \
    ...
    $RESUME_FLAG $MAXSTEPS_FLAG $SNAP_FLAG \
...
# Exp3: when EMA is on, the EMA weights (own checksum+provenance in <tag>/ema/) are the intended eval target.
EMA_CKPT="${OUT_DIR}/${TAG}/ema/final_model.pt"
[ -f "$EMA_CKPT" ] && { CKPT="$EMA_CKPT"; echo "[ddp] evaluating EMA checkpoint: ${CKPT}"; }

# --- official diagnostic eval on ONE GPU (the SDPA backbone + bf16, matching training) ---
# Protocol-faithful utility decode (post gap-analysis): SCORE=0.01 / MAXOBJ=500 feed the full PR tail to the
# devkit (its own min_recall/min_precision=0.1 clip handles the floor) — the 0.1/200 pre-floor deflated mAP.
# The ASR subset re-applies its own tau_clean gate, so eligible_N is unaffected. Comparable to Step-0 re-decode.
SCORE="${SCORE:-0.01}"; MAXOBJ="${MAXOBJ:-500}"
EVAL_OUT="${OUT_DIR}/${TAG}/readiness_diag"; mkdir -p "$EVAL_OUT"
CUDA_VISIBLE_DEVICES=0 python fl_v3/scripts/t4_readiness_eval.py \
    --config "$CONFIG" --checkpoint "$CKPT" --output-dir "$EVAL_OUT" --diagnostic --no-gt-sanity \
    "nuscenes-cache-dir=${TRAINVAL_CACHE}" "precision=bf16" \
    "det-score-threshold=${SCORE}" "det-max-objects=${MAXOBJ}"
echo "===== DDP centralized verdict (DIAGNOSTIC, score=${SCORE} maxobj=${MAXOBJ}) ====="
python3 -c "import json;d=json.load(open('${EVAL_OUT}/benchmark_readiness.json'));print('verdict',d['verdict'],'| mAP',round(d['mAP'],4),'NDS',round(d['NDS'],4),'car_recall',round(d['official_clean_car_recall'],4),'eligible_N',d['eligible_count']);p=d.get('per_class_mean_ap') or {};[print(f'  {k:26s} {v:.4f}') for k,v in sorted(p.items(), key=lambda x:-x[1])]" || true

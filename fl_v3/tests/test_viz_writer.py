"""Viz writer: lays down the clean V1–V4 tree and a deterministic manifest."""
...
def test_viz_tree_created(tmp_path):
    assert VIZ_STAGES == ("calibration", "encoder", "fusion", "detection")
...
def test_write_json_and_manifest_are_deterministic(tmp_path):
    def build(run_dir):
        ...
        w.figure_path("detection", "prediction_overlay")
    ...
    assert data["stages"]["detection"]["viz_id"] == "V4"
    # Registered artifacts are recorded (sorted).
    assert "viz/fusion/b_artifact.json" in data["stages"]["fusion"]["artifacts"]
    assert "viz/detection/prediction_overlay.png" in data["stages"]["detection"]["artifacts"]

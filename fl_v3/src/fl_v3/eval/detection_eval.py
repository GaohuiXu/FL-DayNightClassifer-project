...
def submission_meta() -> Dict[str, bool]:
    """The required devkit submission ``meta`` (modality flags; camera+LiDAR, no radar/map)."""
    return {
        "use_camera": True,
        "use_lidar": True,
        "use_radar": False,
...
@torch.no_grad()
def decode_eval_set(model, eval_loader, device, run_config) -> List[SampleDecode]:
...
def build_results_dict(
    ...
    return {"meta": submission_meta(), "results": results}

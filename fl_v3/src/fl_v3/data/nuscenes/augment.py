...
def sample_transform(p: Dict) -> np.ndarray:
    ...
    theta = float(np.random.uniform(-p["rot"], p["rot"])) if p.get("rot", 0) else 0.0
    ...
    t = np.random.normal(0.0, tr, 3).astype(np.float64) if tr else np.zeros(3, np.float64)
    c, s = np.cos(theta), np.sin(theta)
    Rz = np.array([[c, -s, 0.0], [s, c, 0.0], [0.0, 0.0, 1.0]], dtype=np.float64)
    L = np.diag([fx, fy, 1.0]) @ (scale * Rz)        # linear part: flip · scale · Rz
...
def augment_sample(sample: Dict, params: Dict) -> Dict:
    ...
    pts, boxes, vel, l2i = apply_transform(
        sample["lidar_points"], sample["gt_boxes"], sample["gt_velocity"], sample["lidar2img"], T)

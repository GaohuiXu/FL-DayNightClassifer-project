...
2. Compute per-class mean features `{μ_k^c(r) : c ∈ classes(D_k)}` for every class `c` that the client has at least **5 samples** of. Call these the client's **observable classes**. (The threshold of 5 was chosen via the analysis in [`docs/phaseE2_coverage_analysis.md`](../phaseE2_coverage_analysis.md), 2026-04-27 — `t=5` gives ~99.9% pair coverage versus ~91% at `t=10`, with only mild per-pair noise penalty.)
...
**Why this is non-IID-native.** Each client measures drift *in its own local feature frame*, relative to *its own baseline*. There is no assumption that the client's class distribution matches the global distribution. A client with 30 class-3 samples can detect "class 3 is drifting toward class 2" just as well as a client with 300 class-3 samples — the estimate is noisier but the *direction* is still informative. Non-IID limits which pairs each client can observe, not whether those observations are valid. Pair coverage is measured empirically: at the chosen threshold `t=5` over our 50-client / Dirichlet-α=0.5 partition, 99.9% of the 1806 ordered pairs are covered by ≥1 client and the median covered pair has 10 independent witnesses (see [`docs/phaseE2_coverage_analysis.md`](../phaseE2_coverage_analysis.md)).
...
**Mechanism.** After each client runs its detector, it reports a sparse "top-k suspicious pairs with scores" vector to the server as a side channel (distinct from the weight update — reported via `MetricRecord` alongside the model update). The server aggregates across clients with **per-flag weight `w_k(c, c') = min(count_k(c), count_k(c'))`**, so a client with the bare-minimum 5 samples per endpoint contributes less than a client with 50 — recovering the "more samples = more reliable estimate" intuition without losing coverage. Consensus rule: if the weighted score for an ordered pair `(c → c*)` exceeds threshold and is supported by **≥ 2 independent witnesses**, declare the global model compromised. Decision action: either (i) roll back to the previous round's aggregate or (ii) reject the weight updates from the current round.
...
**Known coverage limitation (rare classes).** The per-pair coverage analysis in [`docs/phaseE2_coverage_analysis.md`](../phaseE2_coverage_analysis.md) shows that the residual ~0.1% of uncovered pairs at `t=5` cluster on GTSRB's rarest classes (notably class 0 and class 42, with ~210 and ~240 training samples). An adaptive attacker who chooses a long-tail target class can systematically evade detection. This is reported as an inherent limit of the FL non-IID setting in Phase F, not a defense flaw. Practical implication for Phase D.2: keep `backdoor-target-label = 2` (well-covered, frequent class) for the primary attack runs; only pick a rare target class if explicitly probing the blind-spot regime.
...
2. ~~**Class-pair coverage in cross-client detection (Phase E.2).**~~ **Resolved 2026-04-27** — see [`docs/phaseE2_coverage_analysis.md`](../phaseE2_coverage_analysis.md). At observable threshold `t=5` over the 50-client Dirichlet α=0.5 partition, 99.9% of the 1806 ordered class pairs are covered by ≥1 client; median redundancy is 10 witnesses per covered pair; the residual blind region clusters on GTSRB's rare classes (0, 42). The threshold was lowered from the originally-assumed 10 to 5 in Phase E.1 Design A as a result.

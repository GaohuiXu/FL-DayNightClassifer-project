...
| [02](cycle_02_gradient_space_mechanism_study.md) | 2026-05-13 → 2026-05-29 | Gradient-space backdoor mechanism study — modern attacks/defenses on the audit-fixed codebase, threat-model design | **closed** | FLAME drives ASR to 0.000 against all 3 static attacks (pixel, model_replacement, DBA); MultiKrum bimodal; FoolsGold evaded by DBA; NormClip a no-op. Stage-study follow-up (`../cycle_02/stage_study_log.md`) quantified FLAME's clean honest-rejection rate at **0.469** — it drops ~half the honest clients every clean round; 0% ASR is real but costly. The "gradient-space law" question is moot until a stronger attacker exists — so the next cycle builds one. |
| [03](cycle_03_stronger_adaptive_attacks.md) | 2026-05-28 → 2026-06-15 | Stronger adaptive attacks — reproduce 2023-25 SOTA (small-LR, LP, A3FL, cond. 3DFed) to find one that breaks FLAME's 0.000 baseline | **paused** | Small-LR (WS-B) did not break FLAME on Adam/GTSRB (continuous-injection peak ASR 0.72 vs FedAvg but evades nothing under FLAME); combined with the Cycle-02 close-out (GTSRB saturated, α=0.2 doesn't fragment the honest cluster), the thesis pivoted to real AD perception. **Paused, not closed** — LP/A3FL/3DFed remain unimplemented; a separate session writes the Cycle-03 summary log. |
| [04](cycle_04_fusion_layer_backdoors.md) | 2026-06-15 → TBD | **Federated multimodal AD perception platform + backdoor attack/defense benchmark** — bit-deterministic camera+LiDAR BEV/3D detection on nuScenes (fresh `fl_v3/` codebase); a fusion-aware backdoor attack suite × a general FL defense suite (FLAME, FoolsGold, MultiKrum, FedMedian, NormClip). Platform-first; not bonded to FLAME. | **active** | — |
